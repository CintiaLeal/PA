...
public class Sistema extends ISistema {
    ...
    public boolean confirmarEspectaculo(String plataforma, String nick, String nombre, String desc, String url, Integer dur, Integer min, Integer max, Double costo, String fecha, Estado estado, ListString c1, String img, Integer cant, String descP) {
        ...
        p.agregarEspectaculo(a, nombre, desc, url, dur, min, max, costo, fecha, estado, c, img, cant, descP);
        em.merge(p);
        t.commit();
        em.close();
        return true;
...
//        List<String> ret = e.listarCategoria();
    ...
    public boolean estaComprado(String nick, String paquete) {
        ...
        PaqueteJpaController pj = new PaqueteJpaController();
        EntityManager em = pj.getEntityManager();
        Query q = em.createNamedQuery("SELECT p.nombre FROM Espectador e JOIN Paquete p ON e.paquetes.nombre = p.nombre WHERE e.nick =:nick AND p.nombre =:paquete");
        q.setParameter("nick", nick);
...
//        }
    ...
    public void crearPDF(String nick, String premio) throws FileNotFoundException, DocumentException {
        try {
            ...
            OutputStream file = new FileOutputStream(new File("C:\\Users\\Administrador\\Desktop\\Laboratorio\\Comprobante_" + fechaPremio + ".pdf"));
