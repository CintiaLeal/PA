...
//        }
    ...
    @Override
    public List<DtAgente> listarAgentes() {
        AgenteJpaController pjpa = new AgenteJpaController();
        List<Agente> agentes = pjpa.findAgenteEntities();
        List<DtAgente> ret = new ArrayList();
        for (int i = 0; i < agentes.size(); i++) {
            if (agentes.get(i).getIp() != null) {
                ret.add(agentes.get(i).getData());
            }
        }
        return ret;
    }
